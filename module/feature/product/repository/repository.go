...
func (r *ProductRepository) GetProductByID(productID uint64) (*entities.ProductModels, error) {
	...
	if err := r.db.Preload("Categories").Preload("ProductPhotos").Preload("ProductReview.User").Where("id = ? AND deleted_at IS NULL", productID).First(&products).Error; err != nil {
		return nil, err
	}
